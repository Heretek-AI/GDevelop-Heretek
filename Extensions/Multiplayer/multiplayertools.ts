  ...
  export namespace multiplayer {
    ...
    const handleJoinLobbyEvent = function (
      ...
      _websocket.onclose = () => {
        ...
        lobbiesIframe.contentWindow.postMessage( // nosonar: typescript:S2819 — wildcard target origin is intentional: the message is not sensitive and a wildcard allows easy debugging.
          {
            id: 'lobbyLeft',
          },
    ...
    const quickJoinWithLobbyID = async (
      ...
      try {
        ...
        const errorCode = parseInt(
          (error as Error).message.match(/\d{3}/)?.[0] || ''
        );
        if (errorCode === 404) {
