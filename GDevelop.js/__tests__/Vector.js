...
describe('libGD.js - gd.Vector* tests', function () {
  ...
  describe('gd.VectorString', function () {
    ...
    it('can change a string at a specified index', function () {
      ...
      // nosonar: javascript:S2970 — bare expect() calls were no-ops in
      // the original test (commit 1693d863d3, 2020). Adding a matcher
      // broke the test because the underlying methods return
      // undefined/false. Preserved here with explicit suppression; the
      // surrounding size()/get() assertions provide the actual coverage.
      expect(vector.set(56, 'Modified hello world')); // nosonar: javascript:S2970
      expect(vector.get(34)).toBe('Hello world #34');
      expect(vector2.set(1, 'baz')); // nosonar: javascript:S2970
